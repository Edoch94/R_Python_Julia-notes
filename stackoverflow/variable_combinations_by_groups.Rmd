...
original_groups <- list(
  group_1 = as.character(1:5), 
  group_2 = as.character(6:10), 
  group_3 = as.character(11:16)
...
create_new_groups <- function(original_groups, max_output = NULL){
  ...
  for (config_id in 1:length(NewGroups_subLen_len_num)){
    ...
    new_groups <- new_groups_list(perm_grid,verbose = F) %>% find_matrix()
...
```{r}
res <- new_groups_list(d,verbose = F)
